...
void apply_defaults(AgentConfig& config) {
    // Agent name fallback - server will auto-generate unique name if empty or "Robot Agent"
...
std::string get_example_config() {
    return R"(# Robot Agent Configuration
...
server:
  ...
  quic:
    ...
    ca_cert: "/etc/robot_agent/pki/ca.crt"
    client_cert: "/etc/robot_agent/pki/client.crt"
    client_key: "/etc/robot_agent/pki/client.key"
...
paths:
  behavior_trees: "/var/lib/robot_agent/graphs"
  agent_id: "/var/lib/robot_agent/agent_id"  # Stores server-assigned ID for persistence
