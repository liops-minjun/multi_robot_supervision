...
class TestActionServerA(LifecycleNode):
    ...
    async def execute_callback(self, goal_handle):
        ...
        if success:
            goal_handle.succeed()
            result.success = True
            result.message = f'Test A completed successfully in {actual_execution_time:.2f}s'
            self.get_logger().info(f'Goal succeeded: {result.message}')
        else:
            goal_handle.abort()
            result.success = False
            result.message = f'Test A failed after {actual_execution_time:.2f}s (simulated failure)'
