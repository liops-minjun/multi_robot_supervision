...
void print_usage(const char* prog_name) {
    std::cout << "Usage: " << prog_name << " [options]\n"
              ...
              << "                       Default: /etc/fleet_agent/agent.yaml\n"
              ...
              << "\n"
              << "Example:\n"
              << "  " << prog_name << " -c /opt/fleet_agent/config/agent.yaml\n"
              << std::endl;
}

void print_version() {
    std::cout << "Fleet Agent C++ v1.0.0\n"
...
std::string get_config_path(int argc, char* argv[]) {
    ...
    // Default path
    return "/etc/fleet_agent/agent.yaml";
}
...
int main(int argc, char* argv[]) {
    ...
    // Load configuration
    fleet_agent::AgentConfig config;
    try {
        config = fleet_agent::load_config(config_path);
    } catch (const fleet_agent::ConfigLoadError& e) {
        std::cerr << "Failed to load configuration: " << config_path << std::endl;
        std::cerr << "Error: " << e.what() << std::endl;
        rclcpp::shutdown();
        return 1;
    } catch (const fleet_agent::ConfigValidationError& e) {
    ...
    fleet_agent::logging::init(config.logging.level, config.logging.file);

    std::cout << "Fleet Agent starting..." << std::endl;
    std::cout << "Agent ID: " << config.agent_id << std::endl;
    std::cout << "Config: " << config_path << std::endl;
    std::cout << "Robots: " << config.robots.size() << std::endl;

    // Create and run agent
    fleet_agent::Agent agent(config);
    int result = agent.run();

    // Cleanup
    std::cout << "Fleet Agent exiting with code: " << result << std::endl;
    rclcpp::shutdown();
