...
const StateActionNode = memo(({ id, data, selected }: NodeProps<StateActionNodeData>) => {
  ...
  const accentClasses = capabilityKind === 'service'
    ...
    : {
      ...
      checkbox: 'text-rose-500 focus:ring-rose-500',
    }
  ...
  const updateParameterEditingMode = useCallback((active: boolean) => {
    ...
    setNodes((nds) => nds.map((n) => {
      ...
      if (active) {
        if (currentlyActive && currentZIndex === PARAMETER_EDITING_Z_INDEX) {
          return n
        }
        return {
          ...n,
          zIndex: PARAMETER_EDITING_Z_INDEX,
          data: {
            ...n.data,
      ...
      if (!currentlyActive && currentZIndex === undefined) {
        return n
      }
  ...
  const handleNodeBlurCapture = useCallback((event: React.FocusEvent<HTMLDivElement>) => {
    ...
    if (nextTarget && nodeRootRef.current?.contains(nextTarget)) {
      return
    }
  ...
  return (
    <div
      ...
      `}
    >
      {/* Drag handle: node moves only when this strip is dragged */}
      <div
        ...
        <div className="px-3 pb-2">
          <label className="flex items-center gap-2 cursor-pointer group" onClick={(e) => e.stopPropagation()}>
            <input
              ...
              onClick={(e) => e.stopPropagation()}
              className={`w-3.5 h-3.5 rounded border-gray-500 bg-elevated ${accentClasses.checkbox} focus:ring-offset-0`}
            />
            <Sparkles className={`w-3 h-3 ${accentClasses.text}`} />
            <span className={`text-[10px] ${accentClasses.text} ${accentClasses.textHover}`}>상태 자동 생성</span>
          </label>
          {autoGenerateStates && jobName.trim() && (
            <div className="mt-1 ml-5 flex flex-wrap gap-1">
              ...
              {autoGeneratedStates.length > 3 && <span className="text-[8px] text-muted">+{autoGeneratedStates.length - 3}</span>}
            </div>
          )}
          {autoGenerateStates && !jobName.trim() && (
      ...
      <div className="border-b border-primary">
        ...
        >
          <div className="flex items-center gap-2">
            ...
            <span className="text-[9px] text-muted">({startStates.length})</span>
          </div>
          {expandedSection === 'start' ? <ChevronUp className="w-3 h-3 text-muted" /> : <ChevronDown className="w-3 h-3 text-muted" />}
        </button>
          ...
          <div className="px-3 pb-2 space-y-1.5">
            {startStates.map((ss, idx) => (
              <div key={ss.id} className="flex flex-wrap items-center gap-1 text-[9px] bg-cyan-500/10 px-2 py-1.5 rounded group">
                ...
                <select value={ss.quantifier || 'self'} onChange={(e) => { e.stopPropagation(); updateStartStateField(ss.id, 'quantifier', e.target.value) }} onClick={(e) => e.stopPropagation()} className={`bg-surface text-[9px] ${accentClasses.text} font-medium focus:outline-none cursor-pointer rounded px-1.5 py-0.5 border ${accentClasses.border}`}>
      ...
      <div className="px-3 py-2 border-b border-primary">
        ...
        <div className="flex flex-wrap gap-1">
          {duringStateTargets.length === 0 ? (
            <span className="text-[9px] text-muted italic">실행 중 상태 없음</span>
          ) : (
          ...
          {endStates.map((endState) => {
            ...
            return (
              <div key={endState.id} className="flex items-center gap-1.5 pr-5 relative group">
                ...
                <select value={outcome} onChange={(e) => { e.stopPropagation(); updateEndState(endState.id, 'outcome', e.target.value) }} onClick={(e) => e.stopPropagation()} className="w-14 px-1 py-0.5 bg-surface border border-primary rounded text-[9px] text-primary focus:outline-none cursor-pointer">
                  {OUTCOME_OPTIONS.map(option => <option key={option.value} value={option.value}>{option.label}</option>)}
                </select>
                <span className="text-[9px] text-muted">→</span>
                <select value={endState.state} onChange={(e) => { e.stopPropagation(); updateEndState(endState.id, 'state', e.target.value) }} onClick={(e) => e.stopPropagation()} className="flex-1 px-1 py-0.5 bg-surface border border-primary rounded text-[9px] focus:outline-none cursor-pointer" style={{ color: stateColor }}>
                  {states.map(s => <option key={s} value={s}>{s}</option>)}
                </select>
                {endStates.length > 1 && (
                  <button onClick={(e) => { e.stopPropagation(); removeEndState(endState.id) }} className="absolute right-0 text-muted hover:text-red-400 transition-colors opacity-0 group-hover:opacity-100">
                    <X className="w-3 h-3" />
                  </button>
                )}
              </div>
