...
| **Retry Block** | 실패 시 직전 액션 재시도(max_retries/backoff_ms) + `in/out/failed/warning` 포트 기반 분기 지원 |
...
## Update Note (2026-03-17, Retry Block 런타임 반영 + 중복 result 처리 보강)

### 증상
- Retry Block을 넣어도 실제 실행에서 재시도 없이 바로 End로 종료되는 문제.

### 수정
1) 서버 deploy 경로 보강  
- 파일: `central_server_go/internal/grpc/raw_quic_handler.go`  
- `on_failure` edge의 `retry/backoff_ms/fallback`를 deploy 시 condition JSON으로 인코딩해 agent에 전달.

2) Agent 런타임 retry 처리 추가  
- 파일:
  - `ros2_robot_agent/include/robot_agent/agent.hpp`
  - `ros2_robot_agent/src/agent.cpp`
- failure 전이에서 condition JSON(`retry/backoff_ms/fallback`)을 읽어:
  - retry 횟수 남으면 같은 step 재실행
  - 소진 시 fallback으로 전이
- same-step retry 시 step resource를 release하지 않도록 보정.

3) DynamicActionClient 중복 goal-response 가드  
- 파일:
  - `ros2_robot_agent/include/robot_agent/executor/typed_action_client.hpp`
  - `ros2_robot_agent/src/executor/dynamic_action_client.cpp`
- 동일 goal response 중복 처리로 `GetResult` 요청이 여러 번 나가던 문제 차단(`result_requested` 플래그).

### 기대 효과
- Retry Block(`max_retries`, `backoff_ms`)이 실제 런타임 동작과 일치.
- 중복 result callback으로 retry 카운트가 비정상 소진되는 현상 완화.

## Update Note (2026-03-17, Retry 표시/실행 일치 보강)

### 1) RTM "현재할당BT"에서도 Retry Block 시각화
- 파일: `central_server/frontend/src/components/BehaviorTreeViewer.tsx`
- 기존:
  - on_failure(retry)가 있어도 현재할당BT에서는 실패 edge만 보여 Retry 구조가 보이지 않음.
- 변경:
  - `transition.on_failure`가 object(`retry/backoff/fallback`)인 경우 retry 노드를 생성해 표시.
  - edge를 `action(failure) -> retry`, `retry -> action(loop)`, `retry -> fallback`로 렌더.

### 2) 실행 가능성 체크에서 action_server strict 매칭
- 파일: `central_server_go/internal/executor/scheduler.go`
- 기존:
  - graph가 특정 `action_server`를 요구해도, agent에 같은 `action_type`만 있으면 실행 가능으로 통과될 수 있었음.
- 변경:
  - `action_server`가 명시되면 해당 server 매칭을 우선/강제(leading slash normalize 지원).
  - server 미존재 시 action_type fallback으로 통과시키지 않음.
- 효과:
  - 실행 버튼은 활성인데 실제 step server가 없어 런타임에서 실패하는 케이스 감소.

### 3) ActionGraph Retry 노드 포트 UI 정리
- 파일: `central_server/frontend/src/pages/ActionGraph/nodes/StateRetryNode.tsx`
- 변경:
  - `in` 라벨 추가
  - `out/failed/warning` 라벨 위치를 바깥쪽으로 조정
  - 노드 최소 크기/포트 위치 보정으로 라벨 겹침 완화

## Update Note (2026-03-17, Retry in-edge 저장 유지 + Retry 노드 컴팩트 UI)

### 1) Retry in-edge 저장 후 사라지는 문제 수정
- 파일: `central_server/frontend/src/pages/ActionGraph/index.tsx`
- 원인:
  - `on_outcomes`가 있는 step에서 retry block 복원 시 failure 입력 edge 재생성 분기가 누락됨.
- 변경:
  - retry block 존재 시(`hasRetryBlock`) failure 입력 edge(`action -> retry(in)`)를 항상 복원하도록 보강.
- 효과:
  - Retry in 연결 후 저장/재로드 시 edge가 사라지지 않음.

### 2) Retry 블록 UI 정리 (요청 반영)
- 파일: `central_server/frontend/src/pages/ActionGraph/nodes/StateRetryNode.tsx`
- 변경:
  - `max_retries`, `backoff_ms` 영역을 2열로 축소
  - 불필요한 "Retry Ports" 설명 박스 제거
  - `in/out/failed/warning` 라벨을 노드 내부로 이동
  - 노드 크기/포트 위치를 컴팩트하게 재조정
- 효과:
  - 라벨이 노드 밖으로 밀려 보이는 현상 완화, 편집 가독성 향상.

## Update Note (2026-03-17, Retry 연결 UX + 현재할당BT Retry compact view)

- `ActionGraph/index.tsx`
  - Action -> Retry 연결 시 source handle 미지정이면 첫 failure-class outcome을 기본 선택하도록 수정.
  - success -> retry 연결은 현재 모델이 `on_failure` 기반이라 저장되지 않으므로 연결을 막도록 가드 추가.
  - Retry 노드 자동 배치 간격 축소.
- `BehaviorTreeViewer.tsx`
  - 현재할당BT Retry 노드를 compact하게 재배치.
  - `in/out/failed/warning` 라벨을 노드 내부 badge로 표시해 연결 구조 가독성 개선.


- 추가 수정(2026-03-17): 현재할당BT에서 retry 입력선(action -> retry in) 누락되던 분기를 보강했고, 사용자 요청대로 retry 포트 방향을 `out=좌측`, `in=우측`으로 재정렬했습니다.

- 추가 수정(2026-03-17): Retry 포트를 `out=좌측`, `in=우측상단`, `failed=우측중단`, `warning=우측하단`으로 통일했고, 현재할당BT에서도 retry 입력선/loop/fallback이 빠지지 않도록 viewer 복원 로직을 한 번 더 보강했습니다.

## Update Note (2026-03-18, Retry 위치 저장 유지 + Warning 포트 제거)

- `central_server/frontend/src/pages/ActionGraph/index.tsx`
  - Retry 블록 위치를 `transition.on_failure.ui`로 저장/복원하도록 수정.
  - 저장 후 Retry 노드가 엉뚱한 위치로 이동하던 문제를 완화.
  - Retry fallback은 `retry-failed` 단일 포트로 정리.
  - End Nodes 팔레트에서 `End (Warning)` 항목 제거.
- `central_server/frontend/src/pages/ActionGraph/nodes/StateRetryNode.tsx`
  - Retry 노드 크기/포트/라벨을 재배치해 `out / in / failed` 위치가 실제 포트와 일치하도록 수정.
  - `warning` 포트 제거.
- `central_server/frontend/src/components/BehaviorTreeViewer.tsx`
  - 현재할당BT Retry 노드도 동일한 포트 구조로 정리.
  - 저장된 Retry 위치 정보가 있으면 viewer에서도 우선 반영.

## Update Note (2026-03-18, Realtime goal resource scope + merged state UI)

- `central_server_go/internal/api/realtime_pddl.go`
  - realtime goal에 `resource_type_id`를 추가해 generic `{{resource.name}}` goal을 특정 resource type instance 범위로 제한할 수 있게 수정.
  - goal binding 시 activation condition뿐 아니라 goal_state placeholder도 함께 고려하도록 보강.
  - realtime session 응답에 planner 기준 merged state인 `effective_state`를 추가.
- `central_server/frontend/src/types/index.ts`
  - `RealtimeGoalRule.resource_type_id`, `RealtimeSession.effective_state` 타입 추가.
- `central_server/frontend/src/pages/PDDL/components/RealtimeGoalEditor.tsx`
  - Realtime goal 편집기에 Resource 범위 selector 추가.
- `central_server/frontend/src/pages/PDDL/index.tsx`
  - Realtime PDDL state 표시를 merged/effective state 카드형 UI로 재구성.
  - raw state details 영역을 접이식으로 분리해 `current_state` / `live_state`를 기존처럼 함께 확인 가능하게 수정.
  - effective_state를 agent / resource(cnc, charger) / misc(alias) 그룹으로 나눠 상단에서 더 읽기 쉽게 표시.

검증:
- `docker run --rm -v /home/minjoon/mcs_dev/multi-robot-supervision/central_server_go:/src -w /src golang:1.24-alpine sh -lc '/usr/local/go/bin/gofmt -w internal/api/realtime_pddl.go && /usr/local/go/bin/go test ./...'`
- `cd central_server/frontend && npx tsc --noEmit`

## Update Note (2026-03-18, Realtime goal runtime scope propagation fix)

- `central_server_go/internal/api/realtime_pddl.go`
  - realtime tick 경로에서 session goal을 복사할 때 빠져 있던 `ResourceTypeID` 전달을 복구했습니다.
  - 이 누락 때문에 CNC 전용 realtime goal이 runtime에서는 type scope 없이 평가되어 `charger01_status=running` 같은 잘못된 goal이 solve로 전달되던 문제를 수정했습니다.
  - `solveProblemSpec()`에 `GoalBinding`을 추가하고, solve 직전에 goal_state에 binding을 한 번 더 적용하도록 보강했습니다.
  - binding 후에도 placeholder가 남아 있으면 바로 에러를 내는 방어 로직을 추가했습니다.

효과:
- Realtime goal의 resource scope가 UI/세션 표시뿐 아니라 실제 planner dispatch 경로까지 일관되게 적용됩니다.
- `goal charger01_status=running is unreachable` 오류가 사라지고 CNC goal이 정상 dispatch 됩니다.

검증:
- `docker run --rm -v /home/minjoon/mcs_dev/multi-robot-supervision/central_server_go:/src -w /src golang:1.24-alpine sh -lc '/usr/local/go/bin/gofmt -w internal/api/realtime_pddl.go && /usr/local/go/bin/go test ./...'`
- backend 재빌드 후 새 realtime session에서
  - `selected_goal_name = idle to running free`
  - `selected_resource_name = cnc01`
  - `active_execution_status = running`
  확인.

## Update Note (2026-03-18, Realtime Agent Task Sequence UI polish + live BT highlight)

- `central_server/frontend/src/pages/PDDL/index.tsx`
  - Realtime Agent Task Sequence를 단순 `이전/지금/이후` 3칸 표시에서, agent별 task 흐름을 더 읽기 쉬운 카드/타임라인 UI로 개선했습니다.
  - agent 카드에 현재 액션, 점유 resource, 이전/현재/다음 task 요약을 함께 노출합니다.
  - 각 task 카드는 status, 현재 step 이름, step progress, error 요약을 표시합니다.
  - task 클릭 시 여는 BT modal이 더 이상 정적인 그래프만 보여주지 않고, 해당 task의 현재 running step / completed step / failed step을 실시간으로 강조해서 보여주도록 연결했습니다.

검증:
- `cd /home/minjoon/mcs_dev/multi-robot-supervision/central_server/frontend && npx tsc --noEmit`

## Update Note (2026-03-18, result_states applied before warning/error branch effects)

- `central_server_go/internal/executor/plan_executor.go`
  - Task completion handling now applies common `result_states` first, then overlays warning/error branch effects if the final runtime message is classified as `w:` or `e:`.
  - Execution order is now:
    - normal success: `result_states`
    - warning: `result_states` -> `warning_result_states`
    - error/failure: `result_states` -> `error_result_states`
  - Message variables for warning/error are still appended in the branch-specific stage.

효과:
- 공통 성공 결과를 먼저 반영하고, warning/error 상황에서만 추가 상태를 override하는 모델로 동작합니다.
- `warning_result_states`, `error_result_states`가 비어 있어도 기존 PDDL 실행에는 문제가 없습니다.

검증:
- `docker run --rm -v /home/minjoon/mcs_dev/multi-robot-supervision:/workspace -w /workspace/central_server_go golang:1.24-alpine sh -lc 'apk add --no-cache git ca-certificates >/dev/null && /usr/local/go/bin/go test ./...'`

## Update Note (2026-03-18, legacy agent.cpp path now propagates warning/error result messages)

- `ros2_robot_agent/include/robot_agent/agent.hpp`
- `ros2_robot_agent/src/agent.cpp`

변경 내용:
- Legacy `Agent` execution path에서도 action/service step result의 `message`를 분류/누적하도록 수정했습니다.
  - `w:` prefix -> warning
  - `e:` prefix -> error
  - non-prefixed failure message -> error로 간주
- Terminal success 도달 시 누적 warning/error를 보고:
  - error 누적 -> failed + `e: ...`
  - warning 누적 -> completed + `w: ...`
  - none -> normal completed
- Terminal failure 및 recovery path 없음 케이스도 누적 error 메시지를 우선 사용합니다.
- Completed 상태의 task update도 `blocking_reason`을 함께 서버로 전송하도록 수정했습니다.

효과:
- `bt_warning_test` 같은 warning test task가 legacy 실행 경로에서도 서버에 `w:`를 전달하므로, MCS가 `warning_result_states`를 정상 적용할 수 있습니다.
- Legacy path와 newer task-executor path의 result message 처리 동작이 일치합니다.

검증:
- `source /opt/ros/humble/setup.bash && source /home/minjoon/ros2_ws/install/setup.bash && cd /home/minjoon/ros2_ws && colcon build --packages-select ros2_robot_agent --symlink-install`

## Update Note (2026-03-18, Realtime Agent Task Sequence compact layout + true live BT step only)

- `central_server/frontend/src/pages/PDDL/index.tsx`
  - Realtime Agent Task Sequence 영역의 불필요한 설명/큰 요약 카드를 줄이고, 상하 높이를 더 낮춘 compact layout으로 조정했습니다.
  - 오른쪽에는 `현재 액션`, `점유 Resource`만 남기고, 이전/현재/다음 task는 작은 pill 형태로 압축해 표시합니다.
  - task 카드의 폭/패딩을 줄이고 task id 직접 노출을 제거해 한 화면에서 더 많은 agent/task 흐름을 볼 수 있게 했습니다.
  - task 클릭 시 열리는 BT modal은 이제 execution 추정값이 아니라, 실제 runtime snapshot(`current_graph_id`, `current_step_id`)이 선택한 BT와 일치할 때만 live step을 강조합니다.
  - completed/failed step 강조도 실제 BT step id와 일치하는 항목만 필터링해서 표시합니다.

효과:
- Realtime Agent Task Sequence가 현재 MCS 다크 테마와 크게 벗어나지 않으면서 더 낮고 읽기 쉬운 형태로 정리됩니다.
- task modal에서 실제로 확실한 runtime BT step 정보가 있을 때만 강조하므로, 잘못된 "실시간 step" 표시를 줄일 수 있습니다.

검증:
- `cd /home/minjoon/mcs_dev/multi-robot-supervision/central_server/frontend && npx tsc --noEmit`

## Update Note (2026-03-18, result message classification + warning/error planning effects)

- task-level planning spec에 `warning_result_states`, `error_result_states`, `warning_message_variable`, `error_message_variable`를 추가했습니다.
- action result message는 다음 규칙으로 분류합니다.
  - empty / arbitrary text: normal success
  - `w:` prefix: warning
  - `e:` prefix: error
- `PlanExecutor`는 runtime task 완료 후 final message prefix를 보고
  - normal success -> `result_states`
  - warning -> `warning_result_states` + `warning_message_variable`
  - error/failure -> `error_result_states` + `error_message_variable`
  를 적용합니다.
- frontend PDDL Config에도 Warning/Error result state와 message variable 입력 UI를 추가했습니다.
- `ros2_robot_agent` runtime은 step result message를 누적해서 terminal success/failure 시 최종 `blocking_reason`을 `w:` / `e:` prefix 규칙에 맞춰 서버로 보냅니다.
...
## Update Note (2026-03-18, PDDL draft/realtime settings isolated per Task Distributor)

- `central_server/frontend/src/pages/PDDL/index.tsx`
  - PDDL page local draft persistence is now scoped **per Task Distributor** instead of one global draft shared by every profile.
  - `selectedBTIds`, `selectedAgentIds`, `goalState`, `initialState`, `realtimeGoals`, `realtimeTickIntervalSec`, and `realtimeSessionId` are stored/restored independently for each distributor.
  - Switching distributors now saves the current distributor draft first, then restores the newly selected distributor draft (or starts from an empty scoped draft if none exists).
  - Added one-time migration from legacy global key `mcs.pddl.draft.v2` so existing MOMA PDDL settings are preserved under the correct distributor scope.
  - The last selected distributor id is persisted separately so reopening the page restores the previous distributor context instead of mixing with an unrelated test profile.

효과:
- `error test`와 `MOMA PDDL`의 realtime loop / PDDL draft 설정이 서로 덮어쓰지 않습니다.
- 기존 MOMA PDDL draft를 망치지 않도록 legacy draft migration을 같이 넣었습니다.

검증:
- `cd /home/minjoon/mcs_dev/multi-robot-supervision/central_server/frontend && npx tsc --noEmit`


## Update Note (2026-03-18, warning/effect sync fix for completed realtime executions)

- `central_server_go/internal/api/realtime_pddl.go`
  - realtime session sync가 완료된 execution을 current_state로 반영할 때 기존에는 항상 `assignment.ResultStates`만 적용하던 문제를 수정했습니다.
  - 이제 completed step snapshot의 `error` 메시지를 다시 분류해서
    - normal success -> `result_states`
    - warning(`w:`) -> `result_states + warning_result_states + warning_message_variable`
    - error(`e:`) -> `result_states + error_result_states + error_message_variable`
    를 current/effective state에 그대로 반영합니다.
- `central_server_go/internal/executor/plan_executor.go`
  - realtime sync가 warning/error 분기를 다시 판별할 수 있도록 step snapshot에 저장하는 `StepExecutionStatus.Error` 값을 정규화된 payload만 저장하던 방식에서 **원본 prefix(`w:` / `e:`) 포함 메시지**를 보존하도록 수정했습니다.

원인:
- warning BT 자체는 이미 `w:` 메시지를 서버까지 전달하고 있었지만,
- realtime session이 completed execution을 current_state로 동기화할 때 warning/error branch effect를 무시하고 `result_states`만 재적용해서 최종 state가 다시 `normal_ok`로 보였습니다.
- 게다가 step snapshot에는 prefix가 제거된 payload만 남아 있어서 realtime sync가 warning/error를 재분류할 수도 없었습니다.

효과:
- `bt_warning_test` 같은 warning 완료 태스크가 realtime loop를 통해 끝나도 최종 current/effective state가 `warning_result_states` 값으로 유지됩니다.
- message variable(`testres01_msg` 등)도 realtime session state에 함께 반영됩니다.

검증:
- `docker run --rm -v /home/minjoon/mcs_dev/multi-robot-supervision:/workspace -w /workspace/central_server_go golang:1.24-alpine sh -lc 'apk add --no-cache git ca-certificates >/dev/null && /usr/local/go/bin/go test ./...'`
- `bt_warning_test` realtime 세션 재실행 확인:
  - `testres01_status = warning_locked`
  - `testres01_msg = warnning_test.`


## Update Note (2026-03-18, realtime failed-step sync now applies error branch/message)

- `central_server_go/internal/api/realtime_pddl.go`
  - failed/cancelled realtime executions를 current/effective state로 동기화할 때 기존에는 completed step만 partial effect에 반영하던 문제를 수정했습니다.
  - 이제 terminal step status가 `failed` / `cancelled`인 경우에도
    - `result_states + error_result_states`
    - `error_message_variable`
    를 함께 current/effective state에 반영합니다.
- 결과적으로 `bt_error_test` 같은 에러 종료 태스크도 realtime loop 이후
  - `agent001_status = error`
  - `agent001_msg = error_test`
  가 current/effective state에 유지됩니다.

원인:
- error task는 plan execution 자체가 `failed`로 끝나기 때문에 realtime session sync가 `applyRealtimePartialEffects()` 경로를 탔습니다.
- 그런데 이 경로는 completed step만 반영하고 failed step은 무시해서, error branch effect와 error message variable이 current/effective state에 남지 않았습니다.

검증:
- `docker run --rm -v /home/minjoon/mcs_dev/multi-robot-supervision:/workspace -w /workspace/central_server_go golang:1.24-alpine sh -lc 'apk add --no-cache git ca-certificates >/dev/null && /usr/local/go/bin/go test ./...'`
- `bt_error_test` realtime 세션 재실행 확인:
  - `agent001_status = error`
  - `agent001_msg = error_test`


## Update Note (2026-03-18, Realtime Control Room panel + per-card default reset)

- `central_server/frontend/src/pages/PDDL/index.tsx`
  - Realtime PDDL Loop 위에 `Realtime Control Room` 섹션을 추가했습니다.
  - agent별 / resource별 핵심 상태를 카드 형태로 정리해서 보여주도록 변경했습니다.
    - `*_status`
    - `*_msg`
  - 각 카드 우측에 `Reset default` 버튼을 추가해서, 해당 agent/resource의 status/msg state를 Task Distributor 기본값으로 되돌릴 수 있게 했습니다.
  - 기존 raw state 확인 UI는 Control Room 안의 접이식 섹션으로 옮겨서 `effective / current / live` 원본 값을 계속 확인할 수 있게 유지했습니다.
- `central_server_go/internal/api/realtime_pddl.go`
  - `POST /api/pddl/realtime-sessions/{sessionID}/reset-state` 엔드포인트를 추가했습니다.
  - reset 시
    - realtime session `current_state`를 요청한 기본값으로 갱신하고
    - 같은 key에 대한 `live_state` overlay도 제거해서
    - UI/effective state에 즉시 reset 결과가 반영되도록 했습니다.
- `central_server/frontend/src/api/client.ts`
  - realtime session state reset API 클라이언트를 추가했습니다.
- `central_server_go/internal/api/router.go`
  - realtime session reset-state 라우트를 등록했습니다.

효과:
- PDDL 화면에서 agent/resource 상태와 메시지를 관제용으로 빠르게 확인할 수 있습니다.
- status/msg가 꼬였을 때 카드 단위로 기본값 reset을 바로 걸 수 있습니다.
- raw state는 필요할 때만 접어서 볼 수 있습니다.

검증:
- `cd /home/minjoon/mcs_dev/multi-robot-supervision/central_server/frontend && npx tsc --noEmit`
- `docker run --rm -v /home/minjoon/mcs_dev/multi-robot-supervision:/workspace -w /workspace/central_server_go golang:1.24-alpine sh -lc 'apk add --no-cache git ca-certificates >/dev/null && /usr/local/go/bin/go test ./...'`
- `cd /home/minjoon/mcs_dev/multi-robot-supervision && docker-compose build go-backend frontend && docker-compose up -d --no-deps go-backend frontend`

## Update Note (2026-03-18, realtime agent location stabilization)

- realtime PDDL에서 live planning location이 순간적으로 `none`으로 들어오면 current_state의 concrete location을 유지하도록 보강했습니다.
- 이 변경으로 `reserved_by` reconcile이 stale CNC lock을 더 안정적으로 해제할 수 있습니다.
- `ros2_robot_agent`는 planning_state 전송 시 `agent_location` 토픽의 최신값을 직접 반영하고, 최근 concrete location을 짧게 유지해 transient dropout이 있어도 `*_location`이 바로 `none`으로 떨어지지 않도록 수정했습니다.


[2026-03-18] agent001 location/planning-state fix
- Fixed ros2_robot_agent planning-state location stability for agent001 recovery/debug.
- Added ROS node-name sanitization so explicit UUID agent IDs no longer crash node startup.
- Added dedicated callback group for direct /agent_location planning-state subscription.
- Changed direct planning location cache to keep last valid discrete location until a new topic value arrives, preventing fallback to none from transient callback starvation.
- MCS realtime reservation reconcile patch remains in place; verified effective/live/current state now show agent001_location=cnc03 and cnc03_reserved_by=agent001 in a realtime debug session.
- Restored local agent001 identity by resetting /tmp/robot_agent/agent_id to 77195da2-3b9e-4868-a3dd-6d2a3458f233 and relaunched with default ros2 launch config.


## Update Note (2026-03-20, selective integration merge)

- `feat/pddl-planning-workflow` 기준으로 integration 브랜치의 안정화 변경을 선별 반영했습니다.
- 추가 반영 내용:
  - execute 시 centralized deployment / auto-deploy 보강
  - multi-agent execute 전 deployment 검증
  - JointState parameter editor 개선(고정 순서, angle unit toggle, input focus 유지)
  - QUIC ping/pong latency 안정화(single in-flight ping)
  - neo4j reboot 대응 compose 조정
- feature 브랜치의 realtime PDDL / retry / warning-error / control-room 기능은 유지했습니다.

## Update Note (2026-03-20, state merge priority policy UI)

- Task Distributor의 `State` 편집 영역 우상단에 **우선순위** 버튼을 추가했습니다.
- 모달에서 패턴 기반으로 effective state merge 우선순위를 설정할 수 있습니다.
  - `{{agent.name}}_status`
  - `{{agent.name}}_mode`
  - `{{agent.name}}_location`
  - `{{agent.name}}_battery_*`
  - `{{agent.name}}_*msg*`
  - `{{resource.name}}_status`
  - `{{resource.name}}_location`
  - `{{resource.name}}_reserved_by`
  - `{{resource.name}}_*msg*`
- 각 패턴은 `Live 우선` / `Planner 우선`으로 저장되며 Task Distributor별로 유지됩니다.
- backend는 `state_merge_policies_json`을 Task Distributor에 저장하고, realtime session의 `effective_state` 계산과 runtime/current merge 양쪽에 같은 정책을 적용합니다.
- 정책 저장 후에는 같은 Task Distributor를 사용하는 active realtime session에도 즉시 반영됩니다.
- Task Distributor profile JSON export/import에도 `state_merge_policies`가 포함됩니다.

## Update Note (2026-03-20, action goal-parameter helper copy removal)
- Action Graph Task 편집기의 Goal Parameters 섹션에서 상단 helper 안내 박스를 제거했습니다.
- `${agent_name}`, `${agent.name}`, `${resource_name}`, `${resource.name}` runtime binding 지원은 유지되고, UI만 더 간결하게 정리되었습니다.

## Update Note (2026-03-20, realtime goal multi resource-type scope)
- Realtime PDDL Loop의 각 goal에서 resource 범위를 단일 선택이 아니라 **여러 resource type 추가 방식**으로 바꿨습니다.
- goal UI에서 선택한 resource type은 chip 형태로 보이고, 필요하면 개별 제거 또는 전체 해제가 가능합니다.
- backend는 `resource_type_ids`를 기준으로 goal binding 대상을 필터링하며, 여러 type을 선택한 경우 해당 인스턴스들의 합집합을 후보로 사용합니다.
- 기존 `resource_type_id` 필드는 하위 호환용으로 계속 읽고/응답에 유지합니다.

## Update Note (2026-03-20, worker-slot realtime profile example)
- Added `examples/task_distributor_profiles/worker_slot_realtime_profile.json`.
- This example profile is meant for PDDL profile import and preconfigures a worker-slot based operation model (3 agents, 2 logical worker slots, CNC + charger resources, realtime goals for release/charge/claim/CNC cycle flow).
- The selected task names in the example assume tasks named:
  - `claim worker slot`
  - `release worker slot`
  - `go to charger and dock`
  - `go to cnc and park`
  - `run cnc service cycle 01`
  - `run cnc service cycle 02`

## Update Note (2026-03-20, worker-slot charger-standby profile example)
- Added `examples/task_distributor_profiles/worker_slot_charge_standby_profile.json`.
- This example profile is intended for a "2 workers + 1 charger standby" operation model where:
  - workers release worker slots when battery goes low,
  - non-workers stay docked at charger,
  - healthy non-workers claim a freed slot and then rejoin CNC work.
